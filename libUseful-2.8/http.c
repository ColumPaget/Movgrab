...
if (Chunk->ChunkSize==0)
{
  ...
  if ((Chunk->BuffLen==0) && (InLen==0)) return(STREAM_CLOSED);
	...
	if ((! ptr) && (InLen==0)) ptr=memchr(vptr,'\r',end-vptr);
	if (ptr)
	{
	  *ptr='\0';
		ptr++;
...
{
char *Proto=NULL, *User=NULL, *Pass=NULL, *Token=NULL, *Args=NULL;

if (strcasecmp(Method,"POST")==0) ParseURL(URL, &Proto, &Info->Host, &Token, &User, &Pass,&Info->Doc, &Args);
...
if (StrValid(ptr)) 
SendStr=MCatStr(SendStr,"User-Agent: ",ptr, "\r\n",NULL);
else if (! ptr) SendStr=MCatStr(SendStr,"User-Agent: libUseful/2.8\r\n",NULL);
...
if (StrValid(Logon) || StrValid(Password)) Info->UserName=CopyStr(Info->UserName, Logon);
S=HTTPTransact(Info);
