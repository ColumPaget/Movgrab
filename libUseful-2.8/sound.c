...
int SoundReformat(char **Out, const char *In, int Len, TAudioInfo *AI)
{
int len, i;
...
if (ptr) AudioInfo->DataSize=atoi(ptr) - 44;
if (AudioInfo->DataSize < 1) AudioInfo->DataSize=Wav.DataSize;
...
TAudioInfo *SoundOpenFile(const char *FilePath)
{
TAudioInfo *AudioInfo=NULL;
char *FourCharacter=NULL;
STREAM *S;

S=STREAMOpen(FilePath,"r");
if (! S) return(NULL);

//using calloc for built-in memset
FourCharacter=(char *) calloc(10, sizeof(char));
...
/*  ------------------------ OSS Functions  -------------------- */
int OpenOSSDevice(const char *Dev, int Type, TAudioInfo *Info)
{
	int fd;
	int val;
...
int OpenOSSInput(const char *Dev, TAudioInfo *Info)
{
return(OpenOSSDevice(Dev,INPUT,Info));
}


int OpenOSSOutput(const char *Dev, TAudioInfo *Info)
{
...
int OSSAlterVolumeType(const char *device, int Type, int delta)
{
...
int OSSAlterVolume(const char *device, const char *type_str, int delta)
{
int type, result=-1;
...
int OSSPlaySoundFile(const char *DevPath, const char *FilePath, int Vol)
{
...
int SoundOpenOutput(const char *Dev, TAudioInfo *Info)
{
int fd=-1;
...
int SoundOpenInput(const char *Dev, TAudioInfo *Info)
{
int fd=-1;
...
int SoundPlayFile(const char *Output, const char *Path, int Vol, int Flags)
{
int result=0, pid=0;
...
int SoundAlterVolume(const char *ReqDev, const char *VolType, int delta)
{
char *device=NULL;
int val;
