...
Head->Flags=Flags;
if (Head->Flags & LIST_FLAG_HASH)
{
	Head->Item=(ListNode *) calloc(LIST_HASH_BUCKETS, sizeof(ListNode));
}
}
...
if (ListStart->Flags & LIST_FLAG_HASH)
{
val=fnv_hash(Name, LIST_HASH_BUCKETS);
Curr=ListGetLast(((ListNode *)ListStart->Item)+val);
}
...
{
	val=fnv_hash(Name, LIST_HASH_BUCKETS);
	Start=((ListNode *) Head->Item)+val;
	Curr=ListGetNext(Start);
