...
char *P_Ptr, *ptr, *S_Start;
	...
	while (*S_PtrPtr <= S_End) 
	{
		...
		case MATCH_FOUND:
			...
			//if not matching substrings then this match must
			//have matched the whole string!
			if (! (*Flags & PMATCH_SUBSTR)) 
			{
			(*S_PtrPtr)++;	
			if (*S_PtrPtr < S_End) return(MATCH_FAIL); 
			}
...
int pmatch_process(char **Compiled, char *String, int len, ListNode *Matches, int Flags)
{
...
int pmatch(char *Pattern, char *String, int Len, ListNode *Matches, int iFlags)
{
...
char *s_ptr, *s_end;
	...
	if (iFlags & PMATCH_SUBSTR)
	{
		for (s_ptr=String; s_ptr < s_end; s_ptr++)
		{
			result=pmatch_process(Compiled, s_ptr, s_end-s_ptr, Matches, Flags);
			Flags |= PMATCH_NOTSTART;
		}
	}
	else result=pmatch_process(Compiled, String, Len, Matches, Flags);
